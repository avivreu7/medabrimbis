...
type Deal = {
  ...
  created_at: string;
  // Based on your schema, add other potential fields
  user_id?: string;
  status?: string;
};
...
export default function CommunityPortfolioPage() {
  ...
  useEffect(() => {
    ...
    const dealChannel = supabase.channel('community-deals-view-live')
      .on('postgres_changes', { event: '*', schema: 'public', table: 'community_deals' }, 
      (payload) => {
        fetchCommunityDeals();
      }).subscribe();
      
    const priceChannel = supabase.channel('price-updates-view-live')
    ...
    >
      <div className="flex justify-center mb-8">
        <div className="bg-blue-600 text-white p-5 rounded-xl shadow-lg w-auto inline-block">
          <h1 className="text-3xl font-bold">התיק הקהילתי של שירן</h1>
        </div>
      </div>

      {error && <div className="max-w-screen-xl mx-auto bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded-lg relative mb-6 shadow-lg">{error}</div>}
      ...
      <div className="space-y-8 max-w-screen-xl mx-auto">
            ...
            {isLoading ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-5 gap-4 text-center">
                    <div>
                        ...
                        <p className="font-bold text-2xl text-green-600">${totalProfit.toFixed(2)}</p>
                    </div>
                    <div>
                        <p className="text-slate-500 text-sm">סה"כ הפסד</p>
                        <p className="font-bold text-2xl text-red-600">${totalLoss.toFixed(2)}</p>
                    </div>
                    <div>
                        <p className="text-slate-500 text-sm">אחוז הצלחה</p>
                        <p className="font-bold text-2xl text-slate-800">{winRate.toFixed(1)}%</p>
                    </div>
                    {/* *** התוספת החדשה *** */}
                    <div>
                        <p className="text-slate-500 text-sm">יחס סיכון/סיכוי</p>
                        <p className="font-bold text-2xl text-slate-800">{averageRiskReward > 0 ? `1:${averageRiskReward.toFixed(2)}` : '-'}</p>
                    </div>
                    <div>
                        <p className="text-slate-500 text-sm">עסקאות פתוחות</p>
                        <p className="font-bold text-2xl text-slate-800">{openDeals.length}</p>
                    </div>
                </div>
            )}
        </div>
        ...
        <div className="bg-slate-50/80 backdrop-blur-sm border border-slate-200/60 rounded-xl shadow-lg">
            ...
            <div className="overflow-x-auto">
                ...
                {openDeals.length === 0 && !isLoading && <p className="text-center text-slate-500 py-8">אין עסקאות פתוחות.</p>}
            </div>
        </div>
        ...
        <div className="bg-green-50/80 backdrop-blur-sm border border-green-200/60 rounded-xl shadow-lg">
             ...
             <div className="overflow-x-auto">
                ...
                {closedProfitDeals.length === 0 && !isLoading && <p className="text-center text-green-700/70 py-8">אין עסקאות סגורות ברווח.</p>}
            </div>
        </div>
        ...
        <div className="bg-red-50/80 backdrop-blur-sm border border-red-200/60 rounded-xl shadow-lg">
            ...
            <div className="overflow-x-auto">
                ...
                {closedLossDeals.length === 0 && !isLoading && <p className="text-center text-red-700/70 py-8">אין עסקאות סגורות בהפסד.</p>}
            </div>
        </div>
      </div>
    </div>
