...
export default function ReferPage() {
  ...
  // הגנה קלה: אם לא מחובר—החזר לדף התחברות/לנחיתה
  useEffect(() => {
    (async () => {
      const { data: { user } } = await supabase.auth.getUser();
      if (!user) router.push('/'); // שנה לנתיב ההתחברות שלך אם יש
    })();
  }, [supabase, router]);
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    startTransition(async () => {
      const result = await submitReferral({
        // שים לב: לא שולחים בכלל referrer_name/email
  ...
  return (
    <div dir="rtl" className="min-h-screen p-6 flex items-center justify-center bg-cover bg-center bg-fixed"
      ...
      <div className="max-w-2xl w-full bg-white/80 backdrop-blur-sm border rounded-xl shadow-lg p-8">
        ...
        <p className="text-slate-600 mb-8 text-center">
          מלא/י את פרטי המומלץ. ההמלצה תישלח מטעמך אוטומטית.
        </p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            ...
            <input
              ...
              onChange={(e) => setRecommendedPhone(e.target.value)}
              className="mt-1 block w-full p-2 border rounded"
            />
          </div>
