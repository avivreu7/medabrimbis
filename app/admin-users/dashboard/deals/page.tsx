...
export default function AdminDealsPage() {
  ...
  useEffect(() => {
    if (user) {
      ...
      const priceChannel = supabase.channel('price-updates-admin')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'daily_portfolio_updates' }, fetchPriceUpdates)
        .subscribe();

      const dealChannel = supabase.channel('deal-updates-admin')
        .on('postgres_changes', { event: '*', schema: 'public', table: 'community_deals'}, fetchDeals)
        .subscribe();
  ...
  // נתוני פתוחות (תצוגה בלבד – לא נכנס לשווי/אחוז)
  ...
  const unrealizedPnl = useMemo(
    () => openDealsWithPriceData.reduce((acc, d) => acc + (d.unrealized_pnl || 0), 0),
    [openDealsWithPriceData]
  );

  // --- חשוב: ממומש בלבד ---
  const realizedNet = useMemo(() => totalProfit - totalLoss, [totalProfit, totalLoss]);

  // שווי נוכחי = יתרה התחלתית + ממומש בלבד (ללא פתוחות)
  const currentBalance = useMemo(() => {
    return (initialBalance || 0) + realizedNet;
  }, [initialBalance, realizedNet]);

  // אחוז שינוי = ממומש / יתרה התחלתית
  const percentageChange = useMemo(() => {
    if (!initialBalance) return 0;
    return (realizedNet / initialBalance) * 100;
  }, [realizedNet, initialBalance]);
  ...
  return (
    <div dir="rtl" className="min-h-screen p-4 sm:p-6 lg:p-8 bg-cover bg-center bg-fixed" style={{ backgroundImage: "url('/images/background.jpg')" }}>
      ...
      {error && (
        <div className="max-w-screen-xl mx-auto bg-red-100 border-red-400 text-red-700 px-4 py-3 rounded-lg mb-6 shadow-lg">
          {error}
        </div>
      )}
      ...
      <div className="flex flex-col lg:flex-row gap-8 max-w-screen-xl mx-auto">
              ...
              <>
                ...
                <div className="border-t border-slate-200/80 pt-4 space-y-2">
                  <div className="flex justify-between items-baseline">
                    <span className="text-slate-600">שווי נוכחי (ממומש):</span>
                    <div className="flex flex-col items-end">
                      <span className={`font-bold text-2xl ${balanceColor}`}>{formatCurrency(currentBalance)}</span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="text-slate-600">רווח/הפסד לא ממומש (מידע):</span>
        ...
        <main className="lg:w-2/3 w-full space-y-8">
          {isLoading && !deals.length && (
            <div className="text-center text-white/80 font-bold text-xl">טוען נתונים...</div>
          )}
                ...
                <tbody>
                  {openDealsWithPriceData.map((d) => (
                    <tr key={d.id} className="border-t border-slate-200/60 hover:bg-slate-100/20 align-middle">
                      ...
                      <td className="p-4 text-slate-600 font-medium">{d.current_price ? formatCurrency(d.current_price) : 'טוען...'}</td>
          ...
          <div className="bg-green-50/80 backdrop-blur-sm border border-green-200/60 rounded-xl shadow-lg">
            <div className="p-6">
              <h3 className="text-xl font-bold text-green-800">עסקאות ברווח ({formatInt(closedProfitDeals.length)})</h3>
            </div>
            <div className="overflow-x-auto">
              <table className="w-full text-sm rtl text-right">
