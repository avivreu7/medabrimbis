...
export default function ReferralsPage() {
  ...
  const handleStatusChange = (
    ...
    setReferrals((current) => current.map((r) => (r.id === referralId ? { ...r, ...newStatus } : r)));
  ...
  // ✨ מייצרים הודעת וואטסאפ שמשלבת את שם הממליץ מתוך הרשומה
  const buildWhatsAppMessage = (referrerName?: string | null) => {
    const name = (referrerName || 'חבר/ה').trim();
    // שימי לב לשורות חדשות – נתמכות היטב בוואטסאפ
    return `היי, זאת שירן דימור 
קיבלתי את המספר שלך מ${name} לגבי קבוצת הוואטסאפ שלי – "מדברים עסקאות", למסחר אחראי בשוק ההון.
מתי יהיה לך נוח שנדבר?`;
  };

  const formatWhatsAppLink = (phone: string | null, referrerName?: string | null) => {
    if (!phone) return '';
    // ניקוי מספר + המרה למבנה בינ"ל (IL)
    let cleanedPhone = phone.replace(/\D/g, '');
    if (cleanedPhone.startsWith('0') && cleanedPhone.length === 10) {
      cleanedPhone = '972' + cleanedPhone.substring(1);
    }
    const message = encodeURIComponent(buildWhatsAppMessage(referrerName));
              ...
              <tbody className="divide-y divide-slate-200">
                {referrals.map((r, index) => (
                  <tr key={r.id} className={`hover:bg-slate-50/50 ${r.is_completed ? 'bg-green-50/50' : ''}`}>
                      ...
                      <div className="flex items-center gap-3">
                        {r.recommended_phone && (
                          <>
                            ...
                            <a
                              href={formatWhatsAppLink(r.recommended_phone, r.referrer_name)}
                              target="_blank"
                    ...
                    <td className="p-4 max-w-xs text-slate-600 truncate" title={r.notes || ''}>
                      {r.notes}
                    </td>
